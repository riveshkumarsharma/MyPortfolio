    ...
    >
      <div className="text-white flex justify-between items-center py-5">
        ...
        <div className="cursor-pointer text-lg font-semibold">
          ...
          <span className="text-white">Rivesh Kumar Sharma</span>
          <span className="text-[#8245ec]"></span>
        ...
        <ul className="hidden lg:flex text-gray-300 space-x-3 lg:space-x-6">
          {menuItems.map((item) => (
            <li
              key={item.id}
              className={`nav-btn cursor-pointer hover:text-[#8245ec] ${
      ...
      <div
        className={`mt-1 overflow-hidden absolute top-16 left-1/2 bg-[#050414]/85  transition-all duration-500 ease-in-out transform -translate-x-1/2 w-4/5
                ${isOpen ? "max-h-100 opacity-100" : "max-h-0 opacity-0"}    
                ${
                  isScrolled
                    ? "backdrop-blur-xl shadow-md "
                    : "shadow-lg backdrop-blur-md"
                } z-50 rounded-lg  lg:hidden`}
      >
